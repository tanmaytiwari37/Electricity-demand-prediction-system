...
export default function RiskBlock({ a }: { a: AlertsResponse }) {
  ...
  const glow = { low: 'glow-good', medium: 'glow-warning', high: 'glow-serious', critical: 'glow-critical' }[a.risk_level]
  return (
    <div className="flex h-full flex-col">
      <div className="label">Peak risk</div>
      <div className={`num mt-1 text-5xl font-bold leading-none tracking-[-0.03em] ${m.text} ${glow}`}>{m.label}</div>
