  ...
  return (
    <svg viewBox="0 0 240 200" width={size} height={(size * 200) / 240} role="img" aria-label="Weather and demand emblem" className="max-w-full">
      <defs>
        ...
        <radialGradient id="wm-sunglow" cx="50%" cy="50%" r="50%">
      ...
      {/* concentric sun */}
      <circle cx="96" cy="86" r="66" fill="url(#wm-sunglow)" />
      ...
      <circle cx="96" cy="86" r="30" fill="#f5a623" fillOpacity="0.2" stroke="#f5a623" strokeOpacity="0.85" strokeWidth="1.5" />
      <circle cx="96" cy="86" r="30" fill="none" stroke="#ffffff" strokeOpacity="0.05" strokeWidth="6" />
      {/* rays, only the upper arc so the curve can pass beneath */}
      {[-150, -120, -90, -60, -30].map((deg) => {
        const r1 = 38, r2 = 50
        const a = (deg * Math.PI) / 180
        return <line key={deg} x1={96 + r1 * Math.cos(a)} y1={86 + r1 * Math.sin(a)} x2={96 + r2 * Math.cos(a)} y2={86 + r2 * Math.sin(a)} stroke="#6b6b74" strokeWidth="1.6" strokeLinecap="round" />
      })}

      {/* demand curve: night trough, afternoon peak, evening peak */}
      <path d="M12 168 C 40 168, 52 150, 70 132 S 104 96, 122 100 S 150 130, 160 122 S 178 92, 200 94 L 200 184 L 12 184 Z" fill="#3987e5" fillOpacity="0.14" />
      ...
      <line x1="12" y1="112" x2="228" y2="112" stroke="#f05a5a" strokeOpacity="0.7" strokeWidth="1" strokeDasharray="5 4" />
      <circle cx="200" cy="94" r="4" fill="#3987e5" stroke="#0d0d10" strokeWidth="2" />

      {/* thermometer */}
      <rect x="182" y="60" width="16" height="92" rx="8" fill="#18181b" stroke="#2f2f35" strokeWidth="1" />
      <rect x="182" y={fillY} width="16" height={tubeBottom - fillY + 10} fill="#f05a5a" fillOpacity="0.9" clipPath="url(#wm-tube)" />
      <circle cx="190" cy="158" r="11" fill="#f05a5a" fillOpacity="0.9" stroke="#2f2f35" strokeWidth="1" />
      {[70, 90, 110, 130].map((y) => <line key={y} x1="200" y1={y} x2="206" y2={y} stroke="#6b6b74" strokeWidth="1" />)}
      <text x="212" y="66" fill="#6b6b74" fontSize="8" fontFamily={FONT}>°C</text>
      <text x="18" y="108" fill="#6b6b74" fontSize="8" fontFamily={FONT} letterSpacing="0.06em">CAPACITY</text>
      <text x="18" y="196" fill="#6b6b74" fontSize="8" fontFamily={FONT} letterSpacing="0.06em">00:00</text>
      <text x="192" y="196" fill="#6b6b74" fontSize="8" fontFamily={FONT} letterSpacing="0.06em" textAnchor="end">23:00</text>
    </svg>
  )
}
