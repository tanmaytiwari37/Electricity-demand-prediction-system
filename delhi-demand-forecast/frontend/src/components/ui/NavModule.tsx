...
export default function NavModule({ to, title, description, metric, metricLabel, icon, tone = 'default' }: Props) {
  return (
    <Link to={to} className="pressable group flex items-center gap-4 rounded-md border border-line bg-surface-1 p-5 hover:border-line-strong hover:bg-surface-2">
      <span className="grid h-10 w-10 shrink-0 place-items-center rounded-md border border-line bg-surface-2 text-ink-2 group-hover:text-ink">{icon}</span>
      ...
      <IconArrow size={16} className="shrink-0 text-ink-3 transition-transform duration-150 group-hover:translate-x-0.5 group-hover:text-ink" />
    </Link>
  )
}
