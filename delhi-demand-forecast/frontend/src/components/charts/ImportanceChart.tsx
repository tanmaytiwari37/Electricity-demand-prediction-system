...
export default function ImportanceChart({ items, top = 10 }: { items: FeatureImportance[]; top?: number }) {
  ...
  return (
    <ResponsiveContainer width="100%" height={Math.max(180, data.length * 30)}>
      <BarChart data={data} layout="vertical" margin={{ top: 4, right: 76, left: 8, bottom: 4 }} barCategoryGap={6}>
        ...
        />
        <Bar dataKey="v" fill={C.forecast} radius={[0, 3, 3, 0]} maxBarSize={18} isAnimationActive={false}>
