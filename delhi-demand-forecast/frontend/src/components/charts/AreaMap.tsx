...
const STATUS_COLOR = { ok: C.ink2, warning: C.warning, critical: C.critical }
...
export default function AreaMap({ feeders, height = 300 }: { feeders: Feeder[]; height?: number }) {
  ...
  return (
    <div className="relative">
      <svg viewBox={`0 0 ${w} ${h}`} className="w-full" role="img" aria-label="Schematic map of DISCOM areas showing allocated, not measured, load">
        <defs>
          <pattern id="gridp" width="24" height="24" patternUnits="userSpaceOnUse">
            <path d="M24 0H0V24" fill="none" stroke={C.grid} strokeWidth="0.8" />
          </pattern>
        </defs>
        <rect width={w} height={h} fill="url(#gridp)" rx="4" />
        <text x={12} y={16} fill={C.tick} fontSize="10" fontFamily="Inter, system-ui, sans-serif">N ↑</text>
        {feeders.map((f) => (
          <g key={f.id}>
            <circle cx={x(f.lon)} cy={y(f.lat)} r={r(f.predicted_mw)} fill={STATUS_COLOR[f.status]} fillOpacity={0.12} stroke={STATUS_COLOR[f.status]} strokeWidth={f.status === 'ok' ? 1.2 : 2.5} />
            <circle cx={x(f.lon)} cy={y(f.lat)} r={3.5} fill={STATUS_COLOR[f.status]} stroke={C.surface} strokeWidth={1.5} />
            <text x={x(f.lon)} y={y(f.lat) + r(f.predicted_mw) + 14} textAnchor="middle" fill={C.ink} fontSize="11" fontWeight="600" fontFamily="Inter, system-ui, sans-serif">{f.discom}</text>
            <text x={x(f.lon)} y={y(f.lat) + r(f.predicted_mw) + 27} textAnchor="middle" fill={C.ink2} fontSize="10" fontFamily="Inter, system-ui, sans-serif">≈ {fmtMW(f.predicted_mw)} · {fmtPct(f.utilization_pct, 0)}</text>
      ...
      <p className="mt-1 text-[10.5px] text-ink-3">Schematic layout from area coordinates. Circle area ∝ allocated load; ring colour = status. Not a GIS map, not a measurement.</p>
    </div>
  )
}
